...
| # | 规则 | 说明 |
| --- | --- | --- |
| ① | **先拉后推** | 每次同步先把云端更新的合并进来，再把"上次成功同步后被改过的行"推上去。合并交给服务端已有的行级 LWW（`ON CONFLICT … WHERE updated_at`），客户端不写合并逻辑。⚠️ 拉取时会**保护「本地已删除、墓碑还没推上去」的行**，否则云端那份还没删除的旧行会把它重新拉回本机，删除动作永远发不出去 |
...
- **触发点齐全**：`FileManager.saveToStorage()` 与 `Progress.saveStats()` 成功后都会 `noteDirty()` —— 新建/删除/重命名/移动文件、以及做题/学课/XP/徽章/模拟考都会自动上传（老版本只有"在编辑器里敲字"才触发，进度根本不上传）。
  ...
  专门盯这个回归。
- **只手动同步**（按用户要求）：**没有**后台定时同步、没有防抖自动上传、没有自动重试。
  本机一有改动（改代码、增删文件、做题、学课、考试）就进入「🟡 未同步」状态，
  点顶栏的「云同步」才真的同步。唯一的例外是**打开页面时同步一次**（这样换设备打开就能看到最新的）。
  失败也不会偷偷重试，而是显示 🔴 并保留本机改动，再点一次即可。
- **顶栏「☁️ 云同步」按钮**：直接显示状态（🟢 已同步 / 🟡 同步中·部分未传 / 🔴 同步失败 / ☁️ 开启同步），
  点一下立即同步并弹出结果；没开启同步时点它会打开云同步面板。
- **学堂草稿也进同步**（重要）：在学堂里做题 / 上课 / 看示例时写的代码，以前**只存本机**，
  换设备就没了 —— 这也是"改了代码点了同步也没用"的根因。现在它和文件一样进同步（`learn` 表），
  本地改过的保留、其余用云端版本，跨设备都能拿到。
- **丢数据不再静默**：服务端会回报本次被丢弃的行数（超限 / 档案对不上），客户端据此显示 🟡 警告。
- **每个文件都能看到同步状态**：左栏「代码宝箱」里，有改动还没上传的文件名后面会带一个 🟡，
  同步成功后自动消失；顶栏按钮则显示整体状态。
- ⚠️ **文件夹要归一化再算签名**（踩过的坑）：老数据 / 老客户端推上来的行 `folder_id` 可能是空的，
  而本地 `FileManager.ensureFolders()` 会把它补成默认文件夹「我的作品」。如果两边算签名时不一致，
  这台设备就会永远认为"本地改过"，**云端对这个文件的更新每次都被跳过** ——
  表现就是"在 A 上改了文件、B 上永远看不到"。现在 `sigFile()` 会把空文件夹统一成 `f_mine`。
...
- ⚠️ **同步拉回来之后，界面必须重画**（踩过的坑，也是「文件内容改了却说同步不成功」的真凶）：
  `applyPull()` 只把远端内容写进 localStorage，之后靠 `refreshUI → FileManager.reload()` 重载。
  但 `reload()` 走的是 `notifyReload()` 而不是 `onChange()`（`onChange` 被云同步当成"本机有改动要标未同步"的信号，
  在那里触发会误标 + 回到上面那个死循环）。**如果只重载不通知界面**，数据其实已经同步下来了，
  文件树和编辑区却还显示旧内容 —— 孩子看到的就是"改了内容却同步不成功"；更糟的是编辑区那份旧内容
  会在 30 秒自动保存 / 关页时被写回 `FileManager`，把刚拉下来的新内容覆盖掉，再同步上去就把别的设备的修改抹了。
  `tools/e2e_sync.mjs` 有两条断言盯这个：同步后**左栏文件树**和**编辑区内容**都要是新的一份。
- ⚠️ **编辑区没有用户新内容时不许回写**：`CodeEditor.hasUserEdits()` 记录"最后装载进来的内容"和当前值是否一致。
  云同步重画后编辑区是被程序装载的（`hasUserEdits() === false`），这时 30 秒自动保存 / `beforeunload` 一律跳过，
  只有孩子真的敲了字才回写。
- ⚠️ **墓碑不能每轮重推**：`applyPull()` 对 `deleted` 的行不再记进 `lastSent`。记了的话下一轮会把它当成
  "本地新删除"再推一次，之后再拉回来再推……每轮同步都在空写云端、`rev` 无限上涨。
- ⚠️ **vfs 只按当前档案发墓碑**：数据文件存在一个全局列表里、同步却按"当前档案"分行，
  切换档案时不能把别的档案的数据文件当成已删除推上去（否则云端那份会被清空）。
...
```bash
node tools/test_cloud_sync.mjs                      # 37 项：失败重发/本地优先/并集/补跑/警告/升级迁移/删除不复活/墓碑不重推
npx wrangler pages dev . --port 8788 &              # 真后端（本地 D1）
PLAYWRIGHT_PATH=… node tools/e2e_sync.mjs http://127.0.0.1:8788   # 35 项：两台设备真链路（含"同步后界面/编辑区必须刷新"）
```
